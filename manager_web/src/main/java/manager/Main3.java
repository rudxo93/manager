...
public class Main3 {
	...
	public static void main(String[] args) {
		...
		memberService.regist(vo1);// 데이터베이스에 넣어주기
		...
		// R (select)
		List<MemberVo> ls = memberService.listAll();// 전체 조회 values()은 collection을 반환함 -> collection을 이용해서 ArrayList 생성
		...
		// 하나의 데이터만 조회
		MemberVo vo = null;
		vo = memberService.read(1); // 1번 회원 조회
		System.out.println(vo);

		vo = memberService.read(4); // 4번 회원 조회
		...
		if (vo != null) { // vo가 null이 아니다 => 검색
			...
			memberService.edit(vo); // 변경값 올리기(현재 vo의 번호에, vo가 들어감)
		}

		vo = memberService.read(1); // 업데이트한 1번 회원 조회
		System.out.println(vo);
		System.out.println("수정 완료!");

		// D (delete)
		memberService.remove(2); // 2번 회원 삭제
